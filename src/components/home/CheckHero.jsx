// This file is superseded by CheckHero.astro — kept for reference only
// Do not import this directly; use CheckHero.astro instead
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
        <div className="grid sm:grid-cols-2 gap-8 sm:gap-12 items-start">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-1 gap-2 sm:gap-3">
            {STATS.map((s) => (
              <div key={s.value} className="bg-white/10 rounded-md px-3 sm:px-6 py-3 sm:py-5 backdrop-blur-sm border border-white/10 text-center sm:text-left">
                <div className="text-2xl sm:text-5xl font-bold text-white leading-none tracking-tight">{s.value}</div>
                <div className="text-primary-200 text-[11px] sm:text-sm mt-1 sm:mt-2 leading-snug whitespace-pre-line">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
